...
const UserInvites = () => {
  ...
  const getUserInvites = async (querys) => {
    const { data } = await bc.auth().getAcademyInvites({ status: 'ACCEPTED,WAITING_LIST', ...querys });
        ...
        <Tooltip title="Resend Invites">
          <IconButton onClick={async () => {
            try {
              ...
              getUserInvites({ limit: 10, offset: 0, status: 'ACCEPTED,WAITING_LIST', ...getParams() });
            } catch (error) {
              console.error(error);
  ...
  return (
    <div className="m-sm-30">
      ...
      <div>
        <SmartMUIDataTable
          title="All User Invites"
          columns={columns}
          items={items}
          view="userinvites?"
          singlePage=""
          historyReplace="/admin/userinvites"
          options={{
            print: false,
          ...
          search={(querys) => getUserInvites({ status: 'ACCEPTED,WAITING_LIST', ...querys })}
