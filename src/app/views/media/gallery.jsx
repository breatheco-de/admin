...
const Gallery = () => {
  ...
  const [query, setQuery] = useState(
    pgQuery.get('like') !== null ? pgQuery.get('like') : '',
  );
  const [type, setType] = useState(
    pgQuery.get('type') !== null ? pgQuery.get('type') : 'all',
  );
  const [categories, setCategories] = useState(
    pgQuery.get('categories') !== null
      ? [...pgQuery.get('categories').split(',')]
      : [],
  );
    ...
    if (e.target.value === 'default') {
      ...
      history.replace(
        `/media/gallery?${Object.keys({ ...pagination, sort: e.target.value })
          .map(
            (key) => `${key}=${{ ...pagination, sort: e.target.value }[key]}`,
          )
          .join('&')}`,
      );
    }
    ...
    if (target.checked) {
      ...
      history.replace(
        `/media/gallery?${Object.keys({
          ...
          .map(
            (key) => `${key}=${
              { ...pagination, categories: tempCategories.join(',') }[key]
            }`,
          )
          .join('&')}`,
      );
