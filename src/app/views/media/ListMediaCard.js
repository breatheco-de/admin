...
const ListMediaCard = ({ product, onOpenDialog, onSelected, isSelected  }) => {
  ...
  return (
    ...
    >
       {selected ? <span className="product-price font-medium bg-white text-green py-1 px-3 m-0">
          <Icon>done</Icon>
        </span> : null}
      <Grid container spacing={2} alignItems="center">
        <Grid item lg={4} md={4} sm={4} xs={12} style={{ padding: 0, width: '200px', height: '250px' }}>
          <img
            src={product.mime.includes('pdf') ? type.pdf : product.mime.includes('video') ? type.pdf : product.thumbnail}
            alt={product.name}
            style={{
              height: '100%',
              width: '100%',
        ...
        <Grid item lg={8} md={8} sm={8} xs={12} className="p-6">
          ...
          <div className="flex justify-between mb-4">
            <span className="text-muted">
              ...
              {' '}
              {product.categories.map((c) => <strong className="border-radius-4 px-2 pt-2px bg-secondary mr-1 text-white">{c.name}</strong>)}
            </span>
          </div>
          <div className="flex justify-between mb-4">
            <span className="text-muted">
