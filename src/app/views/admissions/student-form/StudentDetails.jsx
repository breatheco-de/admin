...
const propTypes = {
  ...
  stdId: PropTypes.string.isRequired,
  openRoleDialog: PropTypes.string.isRequired,
  setOpenRoleDialog: PropTypes.string.isRequired,
};
...
const StudentDetails = ({
  ...
  const updateRole = () => {
    bc.auth()
      .updateAcademyMember(stdId, { role })
      .then(({ data, status }) => {
        if (status >= 200 && status < 300) {
          setRole(roles.find((r) => r.slug === data.role).name);
        } else {
        ...
        <List>
          {roles?.map((role_) => (
            <ListItem
              button
              onClick={() => {
                updateRole(role_.slug);
                setRoleDialog(false);
                setOpenRoleDialog(false);
              }}
              key={role_.id}
            >
              <ListItemText primary={role_.name.toUpperCase()} />
            </ListItem>
          ))}
        </List>
      </Dialog>
