...
const isValidToken = async (accessToken) => {
  ...
  const res = await axios.get(
    `${process.env.REACT_APP_API_HOST}/v1/auth/token/${accessToken}`,
  );
...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      const res1 = await axios._post(
        'Login',
        `${process.env.REACT_APP_API_HOST}/v1/auth/login/`,
        { email, password },
      );
    ...
    const res2 = await axios._get(
      'User',
      `${process.env.REACT_APP_API_HOST}/v1/auth/user/me`,
    );
